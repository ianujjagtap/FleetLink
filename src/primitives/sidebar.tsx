...
function useIsCollapsed() {
  ...
  React.useEffect(() => {
    const checkWidth = () => {
      setIsCollapsed(window.innerWidth < SIDEBAR_COLLAPSE_BREAKPOINT);
    };
    checkWidth();
...
>(
  (
    ...
    // Collapse sidebar by default when below breakpoint, but allow toggling
    // biome-ignore lint/correctness/useExhaustiveDependencies: <>
    React.useEffect(() => {
      if (isCollapsed && openProp === undefined && _open === defaultOpen) {
        _setOpen(false);
      }
    }, [isCollapsed, openProp, defaultOpen]);
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, open, setOpen, isMobile, isCollapsed, openMobile, toggleSidebar]
    );
...
>(
  (
    ...
    // Override collapsible prop if screen width is below collapse breakpoint
    const collapsible = isCollapsed ? "icon" : collapsibleProp;
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <PanelLeft style={{ height: "18px", width: "18px" }} />
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          ...
          className="border border-secondary-foreground/20 bg-background text-foreground"
          {...tooltip}
        />
      </Tooltip>
    );
